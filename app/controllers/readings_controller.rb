 ...
 before_filter :authenticate ,:except => [:create]
  ...
  def bthirty(reading,account_no)
    ...
    @readings.each do |r|
       ...
       account_no << "B30" << "$" << r.account_no  << "$" << r.account_no[0..2] << "$" << r.meter_status[0]  << "$" << r.meter_reading.to_i << "$" << r.created_at.strftime("%d%m%y") << "$" << r.mdi_kva << "$" << rdd << "$" << r.pf << "$" << r.bill_month.to_date.strftime("%Y%m")  << "$" << 1 << "$" << "\n"
     end
   else
      b = Bthirty.first
      ...
      @readings.each do |r|
        ...
        account_no << "B30" << "$" << r.account_no  << "$" << r.account_no[0..2] << "$" << r.meter_status[0]  << "$" << r.meter_reading.to_i << "$" << r.created_at.strftime("%d%m%y") << "$" << r.mdi_kva << "$" << rdd << "$" << r.pf << "$" << r.bill_month.to_date.strftime("%Y%m")  << "$" << b.count << "$"  << "\n"
      end
   end
